...
The GWAS results for the sclerostin pQTLs are extracted from `02_data_gwas_sost_region` using the `03_data_gwas_pqtls.R` R script located in the `scripts` folder. The associations of rs1107747 and rs4793023 with HDL cholesterol and triglycerides are adjusted for rs72836567 using the COJO methodology [@yang2012] to account for the effects of the *CD300LG* gene (see [@sec-ldmat] for further details).[^4] No good proxy variants ($r^2 \geq$ 0.8) were identified for the missing sclerostin pQTLs in GCST006867.  
...
[^4]: The results presented here will differ from those in the article because we use an approximation to COJO, whereas the article uses the GCTA tool.  
